...
<li>$p$ 값: 참값이 0이라고 가정했을 때 지금 자료에서 얻은 정도의 값이 우연히 나타날 확률. 작을수록 "참값이 0"이라는 가정이 자료와 부합하지 않는다</li>
</ul>
...
### <i class="fa-solid fa-table"></i> 앞의 네 변수를 4주 단위로 30개 기간 수집
...
: First ten rows of the ice cream consumption data. The full data set has n = 30 four-week periods and 6 variables.
...
<li>지시변수(indicator) 또는 가변수(dummy variable)로 코딩하여 모형에 넣는다</li>
</ul>
</li>
</ul>
:::
...
<li>이 가운데 <b>4단계</b>를 이어서 자세히 봄. 모형을 세운다고 할 때 <b>세울 수 있는 모형에 어떤 종류가 있는가</b></li>
</ul>
:::
...
정리하면 오늘 배울 모든 기법은 이 표의 어느 한 줄을 채우는 도구입니다. 이 가운데 4단계를 조금 더 들여다보고 가겠습니다. 모형을 세운다고 했는데, 세울 수 있는 모형에는 어떤 종류가 있는지부터 알아야 고를 수 있습니다.
:::

## 4단계 모형 설정: 어떤 모형을 세울 수 있는가
...
::: {.callout-tip appearance="minimal"}
**방금 아이스크림 자료로 훑은 여덟 단계가 각각 어느 장의 주제인가** · 1~3단계는 이 장에서 마무리, 4~8단계가 남은 일곱 장의 내용
:::
...
### <i class="fa-solid fa-link"></i> 분야는 달라도 질문의 형태는 동일
...
### <i class="fa-solid fa-bullseye"></i> 앞 장의 관찰을 식으로
...
여기서 $a_0,a_1$: 임의로 정한 직선의 절편과 기울기, $r_i$: 그 직선에서 $i$번째 점까지의 세로 간격
...
### <i class="fa-solid fa-scale-balanced"></i> 네 직선을 세 기준으로 비교
...
![](../../../output/agent/2026-08-25/fig-s01-r2-decomposition.png){width=88% fig-align="center"}
...
### <i class="fa-solid fa-chart-line"></i> 모형 설정에 앞선 자료 탐색
...
### <i class="fa-solid fa-bullseye"></i> 조건부 기댓값의 설명변수를 여럿으로
...
### <i class="fa-solid fa-pen-to-square"></i> 관측 30개를 하나의 식으로
...
### <i class="fa-solid fa-bullseye"></i> 적합값 $\hat{\mathbf y}$ 를 만드는 행렬
...
### <i class="fa-solid fa-triangle-exclamation"></i> 변수를 더하면 $R^2$ 는 항상 증가
...
### <i class="fa-solid fa-lightbulb"></i> 참값과 오차가 전달된 몫
...
둘째 등호는 추정량이 **참값**과 **오차가 전달된 부분**의 합임을 보여 줌. 자료를 다시 모아 같은 계산을 반복하면 추정값이 조금씩 달라지며, 그 값들이 이루는 분포가 **표집분포(sampling distribution)**
...
### <i class="fa-solid fa-bullseye"></i> 기댓값이 참값과 일치
...
### <i class="fa-solid fa-lightbulb"></i> 세 항의 의미
...
::: {.callout-note appearance="minimal"}
**앞 장에서 여기까지** · 진단에서 잔차가 깔때기 모양으로 퍼지면 등분산 위반. **이 장의 질문** · 그때 어떤 방법을 사용하는가
:::
...
### <i class="fa-solid fa-bullseye"></i> OLS 공식에 $\mathbf W$ 를 넣은 형태
...
<li>따라서 가중치는 그 역수인 $1/w$가 됨. $w$ 가 클수록 신뢰도가 낮은 점임</li>
</ul>
:::
...
::: {.callout-note appearance="minimal"}
**다음 장** · Summary: 여덟 장을 관통한 공식과 기호를 한자리에 모으고, 현장에서 사용할 실무 체크리스트로 정리
:::
