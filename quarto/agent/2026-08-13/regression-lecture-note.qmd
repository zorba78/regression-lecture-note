...
<li>파선 $y=x$ 보다 적합선이 <b>완만함</b>. 이 기울기 차이가 회귀라는 이름의 근거</li>
</ul>
:::
:::
::::
...
같은 자리에서 골턴이 쓴 비유가 훌륭합니다. 포도주가 담긴 통에 물을 일정 비율로 붓는 것과 같다고 했습니다. 원래 도수가 얼마였든 늘 같은 비율로 묽어진다는 뜻입니다. 이 비유에는 유전이 들어갈 자리가 없습니다. 두 값이 완벽하게 붙어 있지만 않으면 어디서나 일어나는 산술적 현상이라는 것, 그것이 골턴이 8년 만에 도달한 결론입니다. 그림에서 점 하나가 한 쌍의 부자입니다. 가로축이 아버지 키, 세로축이 아들 키, 단위는 인치. 여기서 두 가지를 동시에 읽으셔야 합니다. 구름이 오른쪽 위로 기울어져 있다는 점, 그리고 그 구름이 상당히 두껍다는 점. 아버지가 크면 아들도 큰 경향은 분명히 있습니다. 그런데 아버지 키가 똑같아도 아들 키는 제각각입니다. 회귀분석은 이 둘을 모두 다룹니다. 기울어진 경향은 직선으로, 두께는 오차의 크기로 요약합니다. 이 그림의 점은 골턴이 직접 적어 둔 가족 기록입니다. 197 가족, 자녀 898명 중에서 아들 465명을 뽑아 아버지와 짝지었습니다. 파선을 하나 더 그어 두었습니다. y가 x와 같은 직선, 그러니까 아들이 아버지와 똑같은 키가 되는 가상의 선입니다. 파란 적합선이 그보다 완만하다는 점을 봐 주십시오. 오른쪽에서는 적합선이 파선 아래로, 왼쪽에서는 위로 갑니다. 큰 아버지의 아들은 아버지만큼 크지 않고 작은 아버지의 아들은 아버지보다 큽니다. 양쪽 모두 가운데로 끌려옵니다. 이 기울기 차이가 회귀라는 이름이 붙은 바로 그 자리입니다.
:::
...
:::: {.columns}
::: {.column width="55%"}
![(a) Histogram of all sons' heights. (b) Histogram of the heights of sons whose father is about 72 inches.](../../../output/agent/2026-08-24/fig-i02-marginal-vs-conditional.png){width=100%}
:::
...
::: notes
같은 자료를 다른 각도에서 보겠습니다. 왼쪽 히스토그램은 아버지가 누구인지 전혀 따지지 않고 아들 465명의 키만 모은 것입니다. 전체 평균이 69.0인치입니다. 오른쪽은 조건을 하나 걸었습니다. 아버지 키가 대략 72인치인 집안만 골라냈으니, 왼쪽 그림에서 세로로 얇은 조각 하나를 잘라 낸 셈입니다. 이 조각 안의 평균은 70.1인치로 올라가 있습니다. 무엇이 달라졌습니까? 조건 하나를 걸었더니 분포가 통째로 옮겨 갔습니다. 예측 문제로 바꿔 보겠습니다. 아무 정보 없이 아들 한 명의 키를 맞히라면 전체 평균 69인치가 최선입니다. 그런데 아버지가 72인치라는 힌트를 받고도 여전히 69라고 답하시겠습니까? 70인치대 초반으로 고쳐 부르는 편이 낫습니다. 이때 숫자가 두 개 나옵니다. 구간 안에 실제로 들어온 아들들의 평균이 70.1, 자료 전체로 그은 직선에서 읽은 값이 70.6입니다. 앞의 값은 소수의 관측만 평균 낸 것이라 흔들림이 큽니다. 뒤의 값은 자료 전체를 써서 그은 선 위에서 읽었습니다.
:::
...
:::: {.columns}
::: {.column width="62%"}
![Each shaded band is one 1-inch bin; grey bands hold fewer than 10 sons, so no local mean is drawn there.](../../../output/agent/2026-08-24/fig-i03-local-means.png){width=100%}
:::
