...
**9. 슬라이드 일부만 줄일 때는 `::: {style="font-size: 0.8em"}` 로 감쌀 것.** 2026-08-28 실측으로 확인: 부모 40px → 안쪽 div 31.2px(0.78em)이고 raw `<ul>`/`<li>` 까지 상속된다. `line-height: 1.2` 를 같이 주면 여러 줄 블록에서 한 줄 더 벌 수 있다. **주의 두 가지** — (가) `::: {.column width="X%" style="..."}` 처럼 컬럼 div에 직접 `style` 을 달면 Quarto의 컬럼 처리가 깨져 세로로 쌓인다. 반드시 컬럼 **안쪽**에 별도 div로 넣을 것. (나) 같은 길이 `:::` 로도 중첩된다(Pandoc은 맨 안쪽 열린 div를 닫음). 슬라이드 전체에 `.smaller` 를 붙이는 것보다 이쪽이 낫다는 것이 사용자 지시다.

**10. 그림을 키울 때 `{width=X%}` 가 안 먹으면 `.reveal .slide img { max-height: 614px }` 가 이미 걸린 것이다.** 세로가 상한에 닿아 있으면 폭을 줄여도 높이가 안 준다. 그때는 `{height=440}` 처럼 높이를 직접 주면 폭이 비율대로 따라온다(05장 선택 후 재진단에서 확인). 반대로 컬럼 폭을 넓혀 그림을 키우려면 짝이 되는 텍스트 컬럼의 글자를 함께 줄여야 수식이 안 잘린다.
