...
description: 회귀분석 특강 슬라이드 현황과 산출물 위치 (2026-08-24 R 제거·이론 강화본)
type: progress
date: 2026-08-24
---

**현행본(2026-08-24 R 제거·이론 강화)**: 원본 `quarto/agent/2026-08-13/regression-lecture-note.qmd` (276 KB), 렌더본 `presentation/regression-lecture-note.html` (5.7 MB). 정비 직전 백업은 같은 폴더의 `regression-lecture-note.qmd.bak-before-style-pass`.

- **구성**: 총 **152 슬라이드**(139 → 141 → 145 → 149 → 151 → 152, 2026-08-26 `8단계의 적용: 아이스크림 자료` 추가). Introduction 21 / Preliminary Knowledge 18 / Simple Linear Regression 22 / Multiple Linear Regression 30 / Diagnosis & Variable Selection 18 / Weighted Regression 21 / Dummy Variable 13 / Summary 9. 렌더 HTML 기준 161장(장 표지 8 + 타이틀 2 포함). 관련: [[lecture-chapter-structure]]
- **강의 대본**: `::: notes` 159건(장 표지 포함, 전 슬라이드 수록).
...
- **남은 일**: (1) 사용자 검토, (2) 151장 기준 강의 시간 배분 점검. 관련: [[slide-level-feedback]], [[ice-data-year-variable]]
