...
6. 현재 memory 폴더 및 .claudes/skills 내용 참조

## 프레젠테이션 생성 규칙

1. Tip: references에 저장되어 있는 사용자가 생성한 quarto 기반 presentation (html, qmd) 파일들의 스타일을 참고할 것
2. 프레젠테이션 내용은 회귀분석에서 필수적인 이론적 요소들이 필수적으로 들어가야함. 특히 회귀계수에 대한 불확도 추정(표준오차, 신뢰구간, 가우스 마코프 therorem 등) 내용을 상세히 설명할 수 있는 슬라이드 필수
3. "12살 수준"은 의미는 슬라이드 내용은 충분히 전문적이되, 슬라이드 노트에 해당 내용들을 청자들에게 잘 이해할 수 있는 스크립트를 노트에 작성해 달라는 의미임. 
4. 해당 강의는 총 3시간(연속) 분량으
